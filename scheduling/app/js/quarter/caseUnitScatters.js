...
define(function (require, exports, module) {
    function getLocalData(){
        ...
        var tempSeriesData=[];
		//饼图没有[同比、环比]
		var locaData = $.zui.store.get('caseUnitScattersQuarter').period;
	...
	exports.initialization = function (params, ajax, chartConfig) {
		...
		//数据拼接
		var tableData = chartConfig.loadChartAndTbale(params, chartConfig,config, 'caseUnitScattersQuarter', 'bar', 'caseUnitScatters');

		var tableTpl = require('../../tpl/table.html');
		var template = Handlebars.compile(tableTpl);

		//表格模板加载
		$('#caseUnitScattersTable').html(template(tableData));
	}
})
