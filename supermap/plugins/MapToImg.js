...
define(function(require, exports, module){
    ...
    function excute(map){
        ...
        for(var i=0;i<layers.length;){
            if(layers[i].CLASS_NAME == "SuperMap.Layer.Markers"){
                ...
                //处理标绘图层的动画图层和图层不显示
                layers.splice(i,1);
            } else if(layers[i].CLASS_NAME == "SuperMap.Layer.Elements"){
                layers.splice(i,1);
            } else if(layers[i].CLASS_NAME == "SuperMap.Layer.AnimatorVector"){
                layers.splice(i,1);
            } else {
                i++;
            }
        ...
        if(LAYER_COUNT>=LAYER_LENGTH){
            ...
            window.setTimeout(function(){
                ...
                if(canvas.msToBlob){
                    ...
                    mapToImageUrl = canvas.toDataURL();
                    parent.window.mapToImageUrl = mapToImageUrl;
                }
            },30);
        }
    }
    ...
    module.exports = mapToImg;
    window.mapToImg = mapToImg;
});