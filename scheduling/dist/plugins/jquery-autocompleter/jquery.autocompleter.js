...
;(function ($, window) {
    ...
    function _launch(data) {
        ...
        if (!data.empty && data.query.length === 0) {
            ...
            if (typeof data.source === 'object') {
                ...
                data.jqxhr = $.ajax({
                    url:        top.fhzxApolloApi+data.source,
                    dataType:   "json",
                    data:       ajaxData,
                    ...
                    beforeSend: function (xhr) {
                        ...
                        if (!$.isEmptyObject(headers.userId) && !$.isEmptyObject(headers.userToken)) {
                            xhr.setRequestHeader("userId", headers.userId);
                            xhr.setRequestHeader("userToken", headers.userToken);
                        }
    ...
    function _buildList(list, data) {
        ...
        for (var item = 0, count = list.length; item < count; item++) {
            ...
            var label = (data.customLabel && list[item][data.customLabel]) ? list[item][data.customLabel] : list[item].name;
        ...
        if (list.length && data.hint) {
            var hint = ( list[0].name.substr(0, data.query.length).toUpperCase() === data.query.toUpperCase() ) ? list[0].label : false;
