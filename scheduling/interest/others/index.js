define(function (require, exports, module) {
    ...
    $('#edit').on('click', function () {
      ...
      if (!$.isEmptyObject(selects)) {
        if (selects.length == 1) {
          ...
          $.showErrorInfo('只能选择一条要编辑的其他兴趣点信息');
        }
      } else {
        $.showErrorInfo('请选择要编辑的其他兴趣点信息');
      }
    });
  }
    ...
    $('#delete').on('click', function () {
      ...
      if ($.isEmptyObject(selects) || selects.length == 0) {
        ...
        $.showConfirmInfo('删除', '确定要删除选中记录?', function () {
          ...
          ajax.post('interest/others/delete', params, function (response) {
  ...
  function initDownload() {
    $('#download').on('click', function () {
      ajax.download('interest/others/download', {}, function (response) {
        var result = response.data;
        ...
        try {
          ...
          linkElement.setAttribute("download", '其他兴趣点信息导入模板.xls');
  ...
  function initialization() {
    ...
    table.initializationWithSearchParams('tableView', 'interest/others/findPage', tableColumns, ajax, params);
    initActions();
  }
  initialization();
})